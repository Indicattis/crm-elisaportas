...
import { Search, CalendarIcon, TrendingDown, User, Radio, Filter, ChevronRight, AlertCircle } from "lucide-react";
...
export default function Losses() {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, selectedFunnelId, selectedSellerId, selectedReason, dateFrom, dateTo]);
  ...
  const filtered = useMemo(() => {
    ...
    return deals.filter((d) => {
      if (selectedReason !== "all" && (d as any).loss_reason !== selectedReason) return false;
      if (!s) return true;
      return (
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-muted/40">
      <div className="p-6 space-y-8 max-w-7xl mx-auto">
        <div className="glass rounded-2xl p-5 flex items-center gap-3 border border-border/60 shadow-[0_4px_20px_-8px_hsl(var(--foreground)/0.1)]">
          <div className="rounded-xl p-3 bg-destructive/10 ring-1 ring-destructive/20">
            <TrendingDown className="h-6 w-6 text-destructive" />
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">Perdas</h1>
            <p className="text-sm text-muted-foreground">
              Todas as negociações marcadas como perdidas
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl p-4 bg-background/70 border border-border/60 ring-1 ring-primary/10">
            <div className="text-xs text-muted-foreground uppercase tracking-wider">Total de perdas</div>
            <div className="text-2xl font-bold text-primary mt-1">{filtered.length}</div>
          </div>
          <div className="rounded-xl p-4 bg-background/70 border border-border/60 ring-1 ring-destructive/15">
            <div className="text-xs text-muted-foreground uppercase tracking-wider">Valor total perdido</div>
            <div className="text-2xl font-bold text-destructive mt-1">{fmtBRL(totalValue)}</div>
          </div>
        </div>
        ...
        <div className="space-y-2.5">
          {loading ? (
            ...
            pageDeals.map((d, idx) => {
              ...
              const reason = (d as any).loss_reason as string | null;
              return (
                <div
                  key={d.id}
                  ...
                  <div className="flex-1 min-w-0 py-3 pr-3 flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                    ...
                    <div className="flex flex-col md:flex-row items-end md:items-center gap-2 md:gap-4 shrink-0">
                      <div className="text-right">
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground/70">Motivo</div>
                        <div className="mt-0.5">
                          {reason ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-destructive/10 text-destructive text-xs font-semibold">
                      ...
                      <div className="text-right">
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground/70">Data</div>
                        <div className="text-xs md:text-sm font-medium tabular-nums mt-0.5">
                          {format(new Date(d.updated_at), "dd/MM/yyyy", { locale: ptBR })}
                        </div>
                      </div>

                      <div className="text-right">
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground/70">Valor</div>
                        <div className="text-destructive font-bold text-sm md:text-base tabular-nums">
                          {fmtBRL(d.value || 0)}
                        </div>
                      </div>
